...
1. 结论：方向结论、简历定位判断、两个岗位的分维度比较、最多三项关键优势与三项主要风险、简历内容状态、推荐理由、档位与换档条件。匹配分和内容完整度都不渲染成大数字。
2. 行动：双轨行动清单与当前推荐岗位的一份求职叙事稿。简历证明轨默认展示五条优先改写并按经历展开其余,每段项目只问一到两个最相关的待补事实；能力提升轨最多三个任务。切换目标岗位时替换求职叙事稿。
3. 依据：简历证据片段、岗位要求边、岗位关键词对照、证据摘要、缺口集、熟练级差异、对账表、计分公式和岗位切片；再列对齐痕迹、观测中、简历多出来的技能点和已覆盖。关键词只分已有证据、只有提及和简历未找到证据,不显示 ATS 分数或命中率。
