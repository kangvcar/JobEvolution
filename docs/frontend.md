...
idle：居中，宽 `min(28rem, 100%)`。领域、目标岗位、简历投放区（带文本层的 PDF / docx，`accept=".pdf,.docx"`）、解析后的技能修正区、开始分析。投放区可见标题当标签，文件 input `sr-only` 包在 label 里。用户可增删技能点并修改简历明确的熟练级；`.doc`、图片与扫描件走技术篇的 400。
...
4. 解释：逐项列岗位要求、简历命中依据、熟练级差异、最小岗位证据摘要与计分公式；再列对齐痕迹、观测中（对外写「市场开始提，还没进要求，不算缺口」）、简历多出来的技能点、已覆盖。半档 0.5、加分 0.3 只写在这里。
