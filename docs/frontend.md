...
一期不做暗色大屏、采集流墙、边上发粒子、G6 Timebar 演化回放、诊断页单独换肤。工单里原先的「两大签名」已随地图 Notes 撤回；本文把签名改成原型里实际留下的两处：岗位切片画布、诊断四拍。
...
左图右卷。高度 `100dvh - 64px`。左：四领域岗位图（G6，见下）+ 图例（领域 / 成型 / 萌芽）。右 inspector 从上到下：读数（萌芽 / 谱内 / 待审）、新岗位发现故事、既有岗更新故事、发现管线条、技能热度、演化流水。点图上岗位进 `/graph?job=`。数据来 `GET /feed` 与 `GET /jobs`。

不要把采集 SSE 画成流墙。计数是库里的聚合。

### `/graph` 图谱

三栏：220px 岗位列表、画布、280px 定义。左栏筛选：领域、技能类目、适用级别（初 / 中 / 高，对应要求边 `levels`）。原型第三项写成了熟练级三档，实现时改成级别，不按了解 / 熟练 / 精通过滤。搜索框滤岗位名。

中栏只画**当前所选岗位**的类目和技能点，dagre 左→右。点类目折叠/展开技能点。点技能点打开右侧详情并可开证据抽屉。点岗位节点不跳走。主按钮「对照简历」带 `job_id` 去 `/diagnose`。

画布 `role="application"`，进页时 focus。键盘：方向键平移 40px，`+`/`=` 放大 1.12，`-` 缩小，Home `fitView`，Esc 取消选中。鼠标拖画布、滚轮缩放保留。

### `/diagnose` 诊断

一页三态，`data-phase="idle|run|done"`。换岗、换简历、离开本页则中止本次分析。

idle：居中，宽 `min(28rem, 100%)`。领域、目标岗位、简历投放区（PDF/Word，`accept=".pdf,.doc,.docx"`）、开始分析。投放区可见标题当标签，文件 input `sr-only` 包在 label 里。

run：同一表单还在，主按钮 `disabled` 且保持「开始分析」四字，旁加忙碌。四拍 ticker 见签名交互。请求是同步 `POST /diagnose`，四拍是等待动画，不跟服务端分拍。

done：顶一条细栏（岗位名 + 再分析一次）。下左右分栏：左简历预览（会话里的 `preview_text`），右对照报告四组，顺序和标题固定：

1. 判断：AI 总结、档位文案、四格（必备覆盖、半档、经验、学历）、目标岗状态、换档条件。匹配分不渲染成大数字。
2. 定位：对照小图谱、类目覆盖（类目只导航、不对账）、邻近岗（同一简历换岗，仍停在 done）。
3. 行动：学习路径（缺口与半档前 5 项，与技术篇一致；原型演示用了 3，实现跟技术篇）、对账表。
...
候选 / 萌芽 / 成型三列看板。点卡出右侧卷宗（簇、独立源、证据、事件）。下方复用总览同一套管线、热度、升值/贬值、演化流水，数据同源，不要第二份口径。候选卡求职者不可见，只在待审；本页求职者看到萌芽与成型，候选列空态写「候选在待审」。

### `/admin` 管理

口令门过后默认待审列表。每条：类型、岗位或技能点、摘要、确认 / 驳回 / 打开证据。低置信条没有「确认发布」当自动结论，按钮禁用并写「不可直通」。顶栏直通开关 `aria-pressed`，文案「直通关闭 / 直通开启」。开着仍列出自动通过流水。改开关 `PUT /admin/passthrough`。

## 签名交互

### 1. 岗位切片画布

在 `/graph` 中栏。这是产品被记住的那张图：一张岗，类目，技能点。不是四领域全景（全景在总览）。

触发：进入 `/graph`、改筛选、改选中岗位、折叠类目。布局 dagre `rankdir: LR`。选中节点描边 3px accent。节点 `radius: 0` 的 rect。边发丝 + 箭头。颜色从 CSS 变量读成运行时颜色再喂 G6，不要在 JS 里写死 hex。
...
库：AntV G6 v5。React 里用官方「在 React 中使用」的容器模式，或 Graphin 3.x 只当画布壳。吃 G6 v5 原生 `Graph` API。不要 sigma、cytoscape、react-force-graph、ECharts graph 当主体。
...
| 工作台 | `#g6` | 当前岗 → 类目 → 技能点 | 类目折叠；技能点详情 / 证据 |
...
| ≤860 | 诊断 done 改上下：简历在上、最高 40vh；四格改两列 |
...
- 状态不只靠颜色：萌芽有「萌芽」pill，缺口有「缺口」字，升值有「+N pt」。
