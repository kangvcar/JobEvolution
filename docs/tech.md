...
2. **抽取。** DeepSeek JSON，Pydantic 校验。一条 JD 抽出：岗位名、领域、原子技能点列表（各带明确必备/明确加分/未标、熟练级、置信 0–1、原文摘录）。只有原文明确写"必须"、"要求"、"熟练掌握"或列入清晰任职要求时才记明确必备,写"优先"、"加分"时记明确加分,其余提及一律未标。斜杠连接的复合技术表述拆开,原文表达任选时同时抽要求组；沟通、团队协作、学习能力等通用素质丢弃。模型品牌从同句动作抽 API 集成、部署、微调或评测,品牌保留在摘录；没有动作只记观测中。校验失败整单重试一次，再失败进待审并标抽取失败。
