...
`/diagnose` 同步返回完整报告，前端 run 态自己播等待动画。报告字段覆盖方向结论、是否无法区分、简历定位判断、优势与风险、简历内容状态、岗位关键词对照、双轨行动清单、项目证据提示、求职叙事稿、简历证据地图关系、邻近岗位迁移数据、市场信号和判断依据。引用已有事实的模型判断携带简历证据片段 ID；缺失判断携带被检查的简历部分,原文没有的事实只进入待补字段。两个岗位没有决定性差异时返回无法区分及各自换档条件,不让模型强选。学习路径按换档条件排序。匹配分可放在 payload 里给档位函数用，UI 不直接渲染该数字。
