...
def s_cover():
    ...
    # stat row
    xs = [1.0, 3.6, 6.2, 8.8]
    for (x, n, l) in zip(xs, ["53", "3,595", "4 / 17", "≥90%"],
...
# =============================================================== 5 data sources
def s_data():
    s = chrome("04 多源异构数据", "53 个企业官方门户 · 3,595 条去重 JD · 四类问题各有一把确定性的刀",
    ...
    xs = [0.6, 3.6, 6.6, 9.6]
    for x, n, l in zip(xs, ["53", "4 类", "3,595", "≤3"],
...
def s_verify():
    s = chrome("12 可验证性", "三套金标、set-based F1、覆盖率门槛与金标修订规程",
               notes="可验证性。我们建了三套各 100 条的金标：JD 解析、简历提取、匹配缺口集，全部用图谱 Skill.id 标注，"
                     ...
                     "最近一次未 mock 结果：简历提取和匹配都是 1.000，JD 解析 0.930，三项都过了 0.90 线；差距样本和复跑记录写在 summary.md 里，我们没有改金标凑数。"
                     "单测 179 通过，覆盖率 69.67%，高于 60% 门槛，核心管线模块覆盖率都在 80% 以上。")
    rows = [
        ["指标", "赛题要求", "数据集", "最近一次未 mock 结果", "状态"],
        ["JD 解析 F1", "≥0.90", "jd.jsonl 100 条（去重、四领域、17 岗各 ≥3）", "0.930", "达标"],
        ["简历提取 F1", "≥0.90", "resume.jsonl 100 份（单栏 / 双栏、PDF / docx）", "1.000", "达标"],
        ["匹配 F1（缺口集）", "≥0.90", "match_pairs.jsonl 100 对（金标简历 × 目标岗）", "1.000", "达标"],
        ["单元测试", "覆盖率 ≥60%", "pytest --cov（分母排除 collectors、llm/client 网络分支）", "179 passed · 69.67%", "达标"],
    ...
    tb(s, 6.9, 4.9, 5.8, 2.0, [
        ...
        {"text": "JD 从 0.814 提升到 0.930：单段抽取、候选召回只接受精确词表命中、降并发避免截断；剩余差距在领域知识类技能召回", "size": 11, "color": DIM},
...
def s_deliver():
    ...
    rows = [
        ...
        ["6", "单元测试", "tests/ · pytest --cov 179 passed · 69.67%（门槛 60%）· GitHub Actions CI", "已完成"],
...
def s_video():
    ...
    rows = [
        ...
        ["7:00–9:00", "可验证性", "终端跑 pytest --cov 与 app.eval；summary.md", "三套金标、set-based F1、覆盖率 69.67%；三项 F1 均达标"],
...
def s_close():
    ...
    items = [
        ...
        ("可验证", "三套 100 条金标 · set-based F1 · 覆盖率 69.67% · Docker 一键复现"),
    ]
    y = 3.85
    for head, body in items:
