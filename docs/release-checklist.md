...
- 评测基线：金标扫描现与正式技能边界一致，排除通用素质和无动作模型品牌；重建了 100 条 JD、100 条简历和 100 条匹配样本。JD 预测还要求技能可回指原文词表，阻止语义嵌入幻觉；新增候选召回仅接受精确原文词表命中，并复用生产管线。最近一次完整未 mock 结果为 JD 0.814、简历 1.000、匹配 1.000，JD 仍低于 0.90；本轮复跑出现模型输出截断，未将不完整结果计入基线。差距样本和复跑要求已写入 `data/eval-official-only/out/summary.md`，没有修改金标以伪造达标。
- 快照：`data/snapshot/release-2026-09-02.json` 与 `data/snapshot/reviewed.json` 已从官方 ATS 图谱重导出，包含 12 个岗位、1603 项技能、3570 条未撤回证据、2998 个事件、0 个岗位定义和 0 条声明；文件 SHA-256 为 `ce39c39f577368d4d67c641d9748862ff785bdfa485c30c42d356dd6581ff127`，两个快照字节一致，快照内证据均为 `source=ats` 且路径位于 `data/official-only/jd/`。`data/eval-official-only/freeze.json` 哈希仍为 `5194b7b806d8fb48714ad3b9f91fe1556a737d36750fe41ab7946a4aadcec438`。
- 双岗样例：`data/eval-official-only/deliver/dual-diagnose.redacted.json`，仅使用合成、脱敏证据，展示方向并列、最小换档数量和未提及证据的报告结构。
- 人工项：系统 Chrome + Playwright 已完成首屏、五步诊断、证据地图、换档模拟、岗位清单、市场卷宗、管理员认证、打印样式、键盘 ESC、320px 和等效 200% 缩放复核；真实 PDF 与临时无个人信息 docx 均已上传并完成单岗/双岗报告，`.doc` 错误提示已验证。语义快速检查覆盖 10 个交互控件（未标注数 0）、跳转主内容链接、`:focus-visible`，并模拟 reduced-motion/forced-colors。屏幕阅读器和真实扫描件仍留给目标发布环境，因为本机没有读屏器和扫描件样本。
