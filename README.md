...
```
cp .env.example .env
# 默认填 DEEPSEEK_API_KEY、ADMIN_PASSWORD；也可将 `LLM_PROVIDER=bai` 并填 `BAI_API_KEY`，或将 `LLM_PROVIDER=tuzi` 并填 `TUZI_API_KEY`，使用对应的 OpenAI 兼容端点。可选 EMBED_API_KEY（硅基流动，嵌入走 BAAI/bge-m3，缺省回落本地哈希向量）
